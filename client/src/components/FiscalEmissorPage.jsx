...
import { FileText, PlusCircle, FileBarChart, Search, Filter, Download, Upload } from 'lucide-react';
    ...
    <TableCell className="text-right">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="bg-popover border-border text-popover-foreground">
          <DropdownMenuItem className="hover:bg-accent hover:text-accent-foreground">Visualizar</DropdownMenuItem>
          <DropdownMenuItem className="hover:bg-accent hover:text-accent-foreground">Enviar por Email</DropdownMenuItem>
          <DropdownMenuItem className="hover:bg-accent hover:text-accent-foreground">Imprimir DANFE</DropdownMenuItem>
          <DropdownMenuSeparator className="bg-border" />
          <DropdownMenuItem className="text-destructive hover:bg-destructive/10">Cancelar</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </TableCell>
  </motion.tr>
...
const InvoiceCard = ({ invoice, delay }) => (
  ...
  >
    ...
    <div className="text-xs text-muted-foreground space-y-1">
      ...
      <div className="flex justify-between">
        <span>Data:</span>
        <span className="text-foreground">{new Date(invoice.date).toLocaleDateString()}</span>
      </div>
    </div>
    <div className="mt-3 flex justify-end gap-2">
      <Button variant="ghost" size="sm" className="text-primary hover:text-primary/80 text-xs h-8 px-2">Visualizar</Button>
      <Button variant="ghost" size="sm" className="text-amber-600 hover:text-amber-700 text-xs h-8 px-2">DANFE</Button>
      <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive/80 text-xs h-8 px-2">Cancelar</Button>
    </div>
  </motion.div>
);

const FiscalEmissorPage = () => {
  ...
  const filteredInvoices = invoiceData.filter(invoice => {
    ...
    const matchesStatus = statusFilter === "todos" || invoice.status.toLowerCase() === statusFilter;
      ...
      >
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground flex items-center">
            <FileBarChart className="mr-2 sm:mr-3 h-6 w-6 sm:h-8 sm:w-8 text-primary" /> 
            Emissor Fiscal
          </h1>
...
export default FiscalEmissorPage;